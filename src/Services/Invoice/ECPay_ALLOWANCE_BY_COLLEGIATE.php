...
use TsaiYiHua\FunPoint\Constants\FunPointAllowanceNotifyType;
 ...
 *  C2線上開立折讓(通知開立)
 */
class FunPoint_ALLOWANCE_BY_COLLEGIATE implements FunPointService
{
    // 所需參數
    ...
    public function checkExtendString(array $arParameters): array
    {
        ...
        // *固定給定下述預設值
        if( ( $arParameters['AllowanceNotify'] != FunPointAllowanceNotifyType::Sms ) && ( $arParameters['AllowanceNotify'] != FunPointAllowanceNotifyType::Email ) && ( $arParameters['AllowanceNotify'] != FunPointAllowanceNotifyType::All ) && ( $arParameters['AllowanceNotify'] != FunPointAllowanceNotifyType::None ) ) {
        ...
        if($arParameters['AllowanceNotify'] == FunPointAllowanceNotifyType::Email && strlen($arParameters['NotifyMail']) == 0 ) {
            array_push($arErrors, "39:NotifyMail is required.");
        }
        ...
        if( $arParameters['AllowanceNotify'] == FunPointAllowanceNotifyType::Sms && strlen($arParameters['NotifyPhone']) == 0 ) {
            array_push($arErrors, "40:NotifyPhone is required.");
        }

        // 39-40 通知電子信箱、通知手機號碼不能全為空值 (如果狀態為SMS 或 EMAIL)
        if(strlen($arParameters['NotifyPhone']) == 0 && strlen($arParameters['NotifyMail']) == 0 && ( $arParameters['AllowanceNotify'] == FunPointAllowanceNotifyType::Sms || $arParameters['AllowanceNotify'] == FunPointAllowanceNotifyType::Email ) ) {
            array_push($arErrors, "39-40:NotifyMail or NotifyPhone is required.");

        } else {

            // *下述情況通知手機號碼與電子信箱不可為空值(通知類別為A-皆通知)
            if( $arParameters['AllowanceNotify'] == FunPointAllowanceNotifyType::All && ( strlen($arParameters['NotifyMail']) == 0 || strlen($arParameters['NotifyPhone']) == 0 ) ) {
                array_push($arErrors, "39-40:NotifyMail And NotifyPhone is required.");
            }

            // *下述情況通知手機號碼與電子信箱為空值(通知類別為N-皆不通知)
            if($arParameters['AllowanceNotify'] == FunPointAllowanceNotifyType::None && ( strlen($arParameters['NotifyMail']) > 0 || strlen($arParameters['NotifyPhone']) > 0 )) {
