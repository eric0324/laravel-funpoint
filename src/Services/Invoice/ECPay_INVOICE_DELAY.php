...
use TsaiYiHua\FunPoint\Constants\FunPointDelayFlagType;
use TsaiYiHua\FunPoint\Constants\FunPointDonation;
use TsaiYiHua\FunPoint\Constants\FunPointInvType;
use TsaiYiHua\FunPoint\Constants\FunPointPayTypeCategory;
 ...
 *  B延遲開立
 */
class FunPoint_INVOICE_DELAY implements FunPointService
{
    // 所需參數
    ...
    public function checkExtendString(array $arParameters): array
    {
        ...
        if ($arParameters['Print'] == FunPointPrintMark::Yes) {
        ...
        if ($arParameters['TaxType'] == FunPointTaxType::Zero || ($arParameters['TaxType'] == FunPointTaxType::Mix && strpos($arParameters['ItemTaxType'], FunPointTaxType::Zero) !== false)) {
            if ( ( $arParameters['ClearanceMark'] != FunPointClearanceMark::Yes ) && ( $arParameters['ClearanceMark'] != FunPointClearanceMark::No ) ) {
        ...
        // *列印註記僅能為 0 或 1
        if ( ( $arParameters['Print'] != FunPointPrintMark::Yes ) && ( $arParameters['Print'] != FunPointPrintMark::No ) ) {
            array_push($arErrors, "12:Invalid Print.");
        }

        // *若捐贈註記 = '1' (捐贈)時，則VAL = '0' (不列印)
        if ($arParameters['Donation'] == FunPointDonation::Yes) {

            if ($arParameters['Print'] != FunPointPrintMark::No) {
        ...
        if ( ($arParameters['Donation'] != FunPointDonation::Yes ) && ( $arParameters['Donation'] != FunPointDonation::No ) ) {
            array_push($arErrors, "13:Invalid Donation.");
        }

        // *若統一編號有值時，則VAL = '0' (不捐贈)
        if (strlen($arParameters['CustomerIdentifier']) > 0 && $arParameters['Donation'] == FunPointDonation::Yes ) {
        ...
        if ( ( $arParameters['CarruerType'] != FunPointCarruerType::None ) && ( $arParameters['CarruerType'] != FunPointCarruerType::Member ) && ( $arParameters['CarruerType'] != FunPointCarruerType::Citizen ) && ( $arParameters['CarruerType'] != FunPointCarruerType::Cellphone ) ) {
            ...
            if (strlen($arParameters['CustomerIdentifier']) > 0) {
                ...
                if ($arParameters['CarruerType'] == FunPointCarruerType::Member || $arParameters['CarruerType'] == FunPointCarruerType::Citizen ) {
                    array_push($arErrors, "15:Invalid CarruerType.");
                }
            }
        }
        ...
        switch ($arParameters['CarruerType'])
        {
            ...
            case FunPointCarruerType::None:
            case FunPointCarruerType::Member:
            ...
            case FunPointCarruerType::Citizen:
            ...
            case FunPointCarruerType::Cellphone:
        ...
        if ( ( $arParameters['TaxType'] != FunPointTaxType::Dutiable ) && ( $arParameters['TaxType'] != FunPointTaxType::Zero ) && ( $arParameters['TaxType'] != FunPointTaxType::Free ) && ( $arParameters['TaxType'] != FunPointTaxType::Mix ) ) {
        ...
        if (sizeof($arParameters['Items']) == 0) {
            ...
            if(!$bError_Tag) {
                ...
                // *檢查商品課稅別
                // 課稅類別為混合稅率時
                if ($arParameters['TaxType'] == FunPointTaxType::Mix) {
        ...
        if( ( $arParameters['InvType'] != FunPointInvType::General ) ) {
            array_push($arErrors, "27:Invalid InvType.");
        }

        // 30.延遲註記 DelayFlag
        if( ( $arParameters['DelayFlag'] != FunPointDelayFlagType::Delay ) && ( $arParameters['DelayFlag'] != FunPointDelayFlagType::Trigger ) ) {
        ...
        if ( $arParameters['DelayFlag'] == FunPointDelayFlagType::Delay ) {
        ...
        if ($arParameters['DelayFlag'] == FunPointDelayFlagType::Trigger) {
        ...
        // *2016-10-4 修改為僅允許 2
        if( $arParameters['PayType'] != FunPointPayTypeCategory::Ecpay ) {
