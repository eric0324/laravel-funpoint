...
class FunPoint_Invoice_CheckMacValue
{
    /**
     * 產生檢查碼
     */
    static function generate($arParameters = array(), $HashKey = '', $HashIV = '', $encType = 0)
    {
        ...
        if(isset($arParameters)) {
            ...
            // 取代為與 dotNet 相符的字元
            $sMacValue = FunPoint_Invoice_CheckMacValue::Replace_Symbol($sMacValue);

            // 編碼
            switch ($encType) {
                case FunPointEncryptType::ENC_SHA256:
                    $sMacValue = hash('sha256', $sMacValue);	// SHA256 編碼
                    break;

                case FunPointEncryptType::ENC_MD5:
                default:
