...
use TsaiYiHua\FunPoint\Validations\InvoiceValidation;
...
class InvoicePostCollection extends Collection
{
    ...
    public function setPostRawData()
    {
        ...
        foreach($items as $item) {
            ...
            $itemData[$i]['ItemTaxType'] = $item['taxType'] ?? FunPointTaxType::Dutiable;
            $itemData[$i]['ItemAmount'] = $item['price']*$item['qty'];
            $amount += $item['price']*$item['qty'];
            $i++;
        }

        $taxType = $this->attributes['TaxType'] ?? FunPointTaxType::Dutiable;
        $print = $this->attributes['Print'] ?? FunPointPrintMark::Yes;
        $carruerType = $this->attributes['CarruerType'] ?? FunPointCarruerType::None;
        $donation = $this->attributes['Donation'] ?? FunPointDonation::No;
        $invType = $this->attributes['InvType'] ?? FunPointInvType::General;
        $customerIdentifier = $this->attributes['CustomerIdentifier'] ?? '';
    ...
    public function setPostDataForCheckout()
    {
        ...
        $print = $this->attributes['Print'] ?? FunPointPrintMark::No;
        $carruerType = $this->attributes['CarruerType'] ?? FunPointCarruerType::None;
        $donation = $this->attributes['Donation'] ?? '0';
        $invType = $this->attributes['InvType'] ?? FunPointInvType::General;
        $customerIdentifier = $this->attributes['CustomerIdentifier'] ?? '';
