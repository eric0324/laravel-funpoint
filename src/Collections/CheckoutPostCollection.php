...
use TsaiYiHua\FunPoint\Validations\InstallmentValidation;
use TsaiYiHua\FunPoint\Validations\ItemValidation;
use TsaiYiHua\FunPoint\Validations\PeriodAmountValidator;
...
class CheckoutPostCollection extends Collection
{
    ...
    public function setBasicInfo()
    {
        if (empty($this->attributes)) {
            throw new FunPointException('attributes must be set');
        }
    ...
    public function setOrderInfo()
    {
        ...
        if (isset($this->attributes['ItemName']) && isset($this->attributes['TotalAmount'])) {
            ...
            if ($validateItem->count() > 0) {
                throw new FunPointException($validateItem->getMessageBag());
            }
        }
    ...
    public function setPeriodAmount(array $data)
    {
        ...
        if ($data['PeriodAmount'] != $this->get('TotalAmount')) {
            throw new FunPointException('PeriodAmount must the same as TotalAmount');
        }
